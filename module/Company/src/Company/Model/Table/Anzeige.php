...
//               echo $select->getSqlString();
     ...
     * @return ResultSet
     */
    public function fetchAllOutdated($companyId, $display=null){

        return $this->tableGateway->select(
            function(Select $select) use ($companyId, $display) {

                $select = $this->getJoin($select);
                $select = $this->getColumns($select);

                $select->where(Entity\Anzeige::TBL_COL_END_DATE   . '<' . self::ORACLE_TODAY_STRING);
                $select->where(Entity\Anzeige::TBL_COL_CUSTOMER_ID . '=' . $companyId);
